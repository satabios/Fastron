...
base.Tensor = class {
    ...
    constructor(tensor, options = {}) {
        ...
        // NEW: Check global configuration for skipping weights
    ...
    get values() {
        // NEW: Skip loading weights if configured
    ...
    get data() {
        // NEW: Skip loading data if configured
        if (this._skipWeights && !this._weightsSkipped) {
            this._weightsSkipped = true;
            if (typeof window !== 'undefined' && window.NETRON_CONFIG && window.NETRON_CONFIG.logMemorySavings) {
                const size = this.estimateSize();
    ...
    get empty() {
        // NEW: If weights are skipped, consider tensor as having data (metadata exists)
    ...
    toString() {
        // NEW: Return metadata string if weights are skipped
        if (this._skipWeights || this._metadataOnly) {
    ...
    // NEW: Get metadata string without loading weights
    getMetadataString() {
    ...
    // NEW: Estimate tensor size in bytes
    estimateSize() {
        if (!this.type || !this.type.shape || !this.type.shape.dimensions) {
            return 0;
        }
        const dimensions = this.type.shape.dimensions.map(d => typeof d === 'bigint' ? Number(d) : d);
        const elementCount = dimensions.reduce((a, b) => a * b, 1);
        const bytesPerElement = this._getBytesPerElement(this.type.dataType);
        return elementCount * bytesPerElement;
    }
    
    // NEW: Get bytes per element for a data type
    _getBytesPerElement(dataType) {
        if (base.Tensor._dataTypes.has(dataType)) {
            return base.Tensor._dataTypes.get(dataType);
        }
        // Default to 4 bytes (float32)
        return 4;
    }
    
    // NEW: Format size in human-readable format
    formatSize(bytes) {
    ...
    // NEW: Load weights on demand
    async loadWeights() {
        if (!this._skipWeights || this._values !== undefined) {
            return this._values;
        }
        this._skipWeights = false;
        this._read();
        return this._values;
    }
    
    _read() {
        // NEW: Skip reading if weights should be skipped
        if (this._skipWeights) {
            return;
        }
