...
protobuf.StreamReader = class extends protobuf.BinaryReader {
    ...
    _fill(length) {
        ...
        if (!this._buffer || this._position < this._offset || this._position + length > this._offset + this._buffer.length) {
            ...
            const configured = Number.isFinite(this._stream.windowSize) ? this._stream.windowSize : 0x1000000;
            const windowSize = Math.max(0x10000, Math.min(0x1000000, configured));
            const size = Math.min(windowSize, this._length - this._offset);
