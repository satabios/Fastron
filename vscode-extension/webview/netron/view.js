    ...
    async start() {
        try {
            ...
            if (this._host.environment('menu')) {
                ...
                if (this._host.type === 'Electron') {
                    const tools = this._menu.group('&Tools');
                    tools.add({
                        label: 'Compare &Models...',
                        accelerator: 'CmdOrCtrl+Shift+C',
                        execute: async () => await this._host.execute('compare-models')
                    });
                }
            }
    ...
    async toggleLazyRendering() {
        ...
        if (button) {
            if (this._options.lazyRender) {
                ...
                button.title = 'Lazy Rendering: ON (Viewport-based)';
            } else {
                button.classList.remove('active');
                button.title = 'Lazy Rendering: OFF (Full graph)';
            }
        }

        // Save the option
        const options = this._host.get('options') || {};
        options.lazyRender = this._options.lazyRender;
        this._host.set('options', options);

        // Reload the current model with new setting
        if (this._model) {
...
view.Graph = class extends grapher.Graph {
    ...
    build(document, containerElement) {
        const element = containerElement || document.getElementById('target');
        while (element.lastChild) {
            element.removeChild(element.lastChild);
        }
        ...
        // Store element references for use by other methods
        this._containerElement = element;
        this._canvasElement = canvas;
        this._originElement = origin;
    ...
    restore(state) {
        ...
        if (context.length > 0) {
            ...
            // Restoring saved state — scroll to input nodes as before.
    ...
    _getViewportBounds() {
        ...
        const scrollY = container.scrollTop / this._zoom;
        const viewportWidth = container.clientWidth / this._zoom;
        const viewportHeight = container.clientHeight / this._zoom;

        return {
            x: scrollX - translateX,
            y: scrollY - translateY,
            width: viewportWidth,
...
export const Graph = view.Graph;
export const Worker = view.Worker;
